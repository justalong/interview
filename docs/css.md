...
- [介绍一下标准的 CSS 的盒子模型？低版本 IE 的盒子模型有什么不同的？](#介绍一下标准的-css-的盒子模型低版本-ie-的盒子模型有什么不同的)
- [CSS 选择符有哪些？](#2css-选择符有哪些)
- [CSS 优先级算法如何计算？](#6css-优先级算法如何计算)
- [CSS 中哪些属性可以继承？](#5css-中哪些属性可以继承)
- [如何居中 div？](#9如何居中-div)
- [display 有哪些值？说明他们的作用。](#10display-有哪些值说明他们的作用)
- [position 的值 relative 和 absolute 定位原点是？](#11position-的值-relative-和-absolute-定位原点是)
- [CSS3 有哪些新特性？](#12css3-有哪些新特性根据项目回答)
- [请解释一下 CSS3 的 Flex box（弹性盒布局模型）](#13请解释一下-css3-的-flex-box弹性盒布局模型以及适用场景)
- [对 BFC 规范的理解？](#27对-bfc-规范块级格式化上下文block-formatting-context的理解)
- [CSS 优化、提高性能的方法有哪些？](#34css-优化提高性能的方法有哪些)
- [margin 和 padding 分别适合什么场景使用？](#37margin-和-padding-分别适合什么场景使用)

#### 不是很必要

- [::before 和:after 中双冒号和单冒号有什么区别？](#3before-和after-中双冒号和单冒号有什么区别解释一下这-2-个伪元素的作用)
- [伪类与伪元素的区别](#4伪类与伪元素的区别)
- [关于伪类 LVHA 的解释?](#7关于伪类-lvha-的解释)
- [CSS3 新增伪类有那些？](#8css3-新增伪类有那些)
- [用纯 CSS 创建一个三角形的原理是什么？](#14用纯-css-创建一个三角形的原理是什么)
- [浏览器是怎样解析 CSS 选择器的？](#35浏览器是怎样解析-css-选择器的)
- [为什么不建议使用统配符初始化 css 样式？](#40为什么不建议使用统配符初始化-css-样式)
- [position:fixed;在 android 下无效怎么处理？](#53positionfixed在-android-下无效怎么处理)
- [什么是 Cookie 隔离？](#60什么是-cookie-隔离或者说请求资源的时候不要让它带-cookie-怎么做)

#### 深度应用

- [CSS 多列等高如何实现？](#16css-多列等高如何实现)
- [display、position 和 float 的相互关系？](#25displayposition和float的相互关系)
- [margin 重叠问题的理解。](#26margin-重叠问题的理解)
- [IFC 是什么？](#28ifc-是什么)
- [清除浮动的方式](#29请解释一下为什么需要清除浮动清除浮动的方式)
- [使用 CSS 预处理器吗？](#33使用-css-预处理器吗喜欢哪个)
- [CSS 里的 visibility 属性有个 collapse 属性值？](#21css-里的-visibility-属性有个-collapse-属性值是干嘛用的在不同浏览器下以后什么区别)
- [width:auto 和 width:100% 的区别](#22widthauto-和-width100的区别)
- [base64 的使用](#24简单介绍使用图片-base64-编码的优点和缺点)
- [style 标签写在 body 前和 body 后的区别](#61style-标签写在-body-后与-body-前有什么区别)
- [rem 布局的优缺点](#64使用-rem-布局的优缺点)
- [常见的 CSS 布局单位](#65几种常见的-css-布局)
- [画一条 0.5px 的线](#66画一条-05px-的线)

#### 开放性问题

- [经常遇到的浏览器的兼容性有哪些？](#17经常遇到的浏览器的兼容性有哪些原因解决方法是什么常用-hack-的技巧)
- [抽离样式模块怎么写？](#38抽离样式模块怎么写说出思路有无实践经验阿里航旅的面试题)
- [什么是响应式设计？响应式设计的基本原理是什么？](#45什么是响应式设计响应式设计的基本原理是什么如何兼容低版本的-ie待深入了解)
- [视差滚动效果？](#46视差滚动效果如何给每页做不同的动画回到顶部向下滑动要再次出现和只出现一次分别怎么做)
- [如果需要手动写动画，你认为最小时间间隔是多久？](#54如果需要手动写动画你认为最小时间间隔是多久为什么阿里)
- [如何实现单行／多行文本溢出的省略？](#97如何实现单行多行文本溢出的省略)
- [css 实现上下固定中间自适应布局？](#99css-实现上下固定中间自适应布局)
- [css 两栏布局的实现？](#100css-两栏布局的实现)
- [css 三栏布局的实现？](#101css-三栏布局的实现)
...
行计算大小和组织，要么依赖于父元素来计算尺寸和组织。当一个元素的hasLayout属性值为true时，它负责对自己和可
...
能实现。所以又提出了ideal viewport的概念，ideal viewport的宽度等于移动设备的屏幕宽度，只要在css中把某一元
素的宽度设为ideal viewport的宽度（单位用px），那么这个元素的宽度就是设备屏幕的宽度了，也就是宽度为100%的效果。i
deal viewport的意义在于，无论在何种分辨率的屏幕下，那些针对ideal viewport而设计的网站，不需要用户手动缩放，也
不需要出现横向滚动条，都可以完美的呈现给用户。
```
...
（2）GIF是无损的、采用索引色的、点阵图。采用LZW压缩算法进行编码。文件小，是GIF格式的优点，同时，GIF格式还
具有支持动画以及透明的优点。但，GIF格式仅支持8bit的索引色，所以GIF格式适用于对色彩要求不高同时需要文件体积
较小的场景。
...
支持动画和透明度。但由于它使用的是索引色，所以它适用于一些对颜色要求不高且需要文件体积小的场景。
...
```
将一个页面涉及到的所有图片都包含在一张大图中去，然后利用CSS的background-image，background-repeat，background
...
用，就是通过改变“行距”来实现的。在CSS中，“行距”分散在当前文字的上方和下方，也就是说即使是第一行文字，其上方也是
